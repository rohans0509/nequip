...
This module handles loading, saving, and updating configuration files (YAML)
and generates the layer irreps string based on model hyperparameters.
"""
...
def irreps(lvalue: int, num_features: int = 32, even: bool = False) -> str:
    """
    Generate an irreps string for e3nn.
    """
...
class ConfigManager:
    def __init__(self, template_dir: str = "src/config_templates"):
    ...
    def update_config(config: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        """Update the configuration dictionary with new key-value pairs."""
    ...
    def generate_layer_irreps(self, lmax: int, num_features: int, inv_layers: int) -> str:
        """
        Generate a layer irreps string based on model hyperparameters.
        For lmax==0, the network is fully invariant.
        """
        params = {
            "lmax": lmax,
